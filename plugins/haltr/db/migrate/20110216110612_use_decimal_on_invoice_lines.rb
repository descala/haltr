class UseDecimalOnInvoiceLines < ActiveRecord::Migration
  def self.up
    # InvoiceLine.all.collect {|i| i.price = (i.price/100).to_s ; i.save(:validate=>false) }
    # InvoiceLine.all.collect {|i| i.quantity = i.quantity.round(2).to_s ; i.save(:validate=>false) }
